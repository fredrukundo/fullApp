...
import { StyleSheet, Text, View, Image, TouchableOpacity, Pressable, Linking, ScrollView, ActivityIndicator } from 'react-native';
import { FontAwesome5, MaterialCommunityIcons, MaterialIcons, Ionicons } from '@expo/vector-icons';
import { useNavigation } from '@react-navigation/native';
import { colors } from '../../../../../Config/theme/colors';
import { ThemeContext } from '../../../../../contexts/ThemeContext';
...
const DetailsScreen = ({ route }) => {
  ...
  if (!trip) {
    return <Text>No trip details found</Text>;
  }
  ...
  return (
    <ScrollView>
      <View style={[styles.container, { backgroundColor: activeColors.bgcolor }]}>
        <Text style={styles.Titles}>Departure:</Text>
        <View style={styles.Departuredates}>
          <Text style={[styles.dateTexts, { color: activeColors.TextColor }]}>{formatDate(trip.departureTime)}</Text>
        </View>
        <View style={styles.FromAndTO}>
          <View style={styles.from}>
            ...
            <View style={styles.fromDetails}>
              <View style={styles.cityFlag}>
                <Text style={[styles.city, { color: activeColors.TextColor }]}>{trip.from.city}</Text>
                <Image source={{ uri: trip.from.flagPhoto }} style={styles.flags} />
              </View>
              <Text style={styles.country}>{trip.from.country}</Text>
            </View>
          </View>
          <View style={styles.to}>
            ...
            <View style={styles.toDetails}>
              <View style={styles.cityFlag}>
                <Text style={[styles.city, { color: activeColors.TextColor }]}>{trip.to.city}</Text>
                <Image source={{ uri: trip.to.flagPhoto }} style={styles.flags} />
              </View>
              <Text style={styles.country}>{trip.to.country}</Text>
            </View>
          </View>
        </View>
        <View>
          <VerifiedButton text="Verified Air Ticket" iconName="verified" iconColor="#0A64EF" borderColor="#dc661f" />
        </View>
        <Text style={[styles.Titles2, { color: activeColors.TextColor }]}>Details</Text>
        <View style={[styles.detailBox, { backgroundColor: activeColors.bgcolor }]}>
          <View style={[styles.detailItem, { borderBottomWidth: 1, borderBottomColor: 'gray', paddingBottom: 10 }]}>
            ...
            <Text style={[styles.itemsText, { color: activeColors.TextColor }]}>Arrival</Text>
            <View style={styles.Arrivaldates}>
              <Text style={[styles.ArrivaldateTexts, { color: activeColors.TextColor }]}>{formatDate(trip.arrivalTime)}</Text>
            </View>
          </View>
          <View style={[styles.detailItem, { borderBottomWidth: 1, borderBottomColor: 'gray', paddingBottom: 10 }]}>
            <MaterialCommunityIcons name="weight-kilogram" size={24} color="#93211E" />
            <Text style={[styles.itemsText, { color: activeColors.TextColor }]}>Number of kilos</Text>
            <Text style={[styles.ArrivaldateTexts, { marginLeft: 'auto', color: activeColors.TextColor }]}>{trip.remainingWeight} kg</Text>
          </View>
          <View style={[styles.detailItem, { borderBottomWidth: 1, borderBottomColor: 'gray', paddingBottom: 10 }]}>
            <FontAwesome5 name="money-check-alt" size={24} color="#616B1B" />
            <Text style={[styles.itemsText, { color: activeColors.TextColor }]}>Price for one kilo</Text>
            <Text style={[styles.ArrivaldateTexts, { marginLeft: 'auto', color: activeColors.TextColor }]}>${trip.price}</Text>
          </View>
          <View style={[styles.detailItem, { flexDirection: 'row' }]}>
            <FontAwesome5 name="luggage-cart" size={24} color="#E0D26A" />
            <Text style={[styles.itemsText, { color: activeColors.TextColor }]}>I sell my kilos in detail</Text>
        ...
        {/* <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center' }} onPress={() => navigation.navigate('User Profile', { Id: trip.user.id, post: trip })}>
          <View style={styles.profileDetails}>
            <Image source={{ uri: trip.user.profilePhoto }} style={styles.profileImage} />
            <View>
              <View style={{ flexDirection: 'row' }}>
                <Text style={[styles.name, { color: activeColors.TextColor }]}>{trip.user.lastName}</Text>
                <MaterialIcons name="verified" size={18} color="#0A64EF" />
              </View>
              <View style={{ flexDirection: 'row' }}>
                <MaterialIcons name="star" size={20} color="#ABB2B9" />
        ...
        <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center' }} onPress={() => navigation.navigate('User Profile', { uzer: trip.user})}>
          <View style={styles.profileDetails}>
            {trip.user.profilePhoto ? (
              <Image source={{ uri: trip.user.profilePhoto }} style={styles.profileImage} />
            ) : (
              <FontAwesome5 name="user-circle" size={40} color="gray" style={styles.profileImage} />
            )}
            <View>
              <View style={{ flexDirection: 'row' }}>
                <Text style={[styles.name, { color: activeColors.TextColor }]}>{trip.user.lastName}</Text>
        ...
        <View style={{ alignItems: 'center' }}>
          <CustomButton text="Book" buttonStyle={styles.button} textStyle={styles.buttonText} onPress={handlePressBook} />
        </View>
        <Pressable onPress={handleReport} style={styles.reportProfile}>
          <Text style={styles.reportProfileText}>Report this Profile</Text>
        </Pressable>
